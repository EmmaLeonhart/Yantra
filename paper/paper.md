...
This paper is the design synthesis for Yantra. As of v3 it is no longer purely a position paper: a v0.0 implementation nucleus exists in the public repository (Connectome Manager + axon router + kernel-services + first userspace utility, 70 tests passing — see § 8.1 for the full status). The thesis and commitments below are unchanged from earlier revisions; what changed is that the load-bearing pieces are now demonstrably implementable, not aspirational.

The empirical claims Yantra's design *rests on* are not invented in this paper: substrate-independent rotation binding, end-to-end-differentiable training of compiled symbolic programs, and substrate-pure beta reduction to tensor normal form are all measured in the Sutra paper across four embedding substrates (100% bundle decoding through width $k=8$, single-cycle bind/unbind round-trip at $\approx 1.5 \times 10^{-15}$, fuzzy-rule training from 4% (random init) to 95% in 50 epochs on a 19-AND-deep rule tree over 992 words / 20 classes). What this paper does is lift those measured primitives into an OS-shaped design and articulate the additional commitments that lifting entails.
...
This repository now holds **planning documents plus a v0.0 implementation nucleus**. The position-paper framing is unchanged, but several of the architecture's load-bearing pieces have shipped end-to-end since the v2 of this paper:

- **Connectome Manager v0.0 (`kernel/`).** Python orchestration layer with admission control against a fixed budget pool, axon router with capability check on send + receive, lazy axon evaluation (skip-uninterested-receivers + per-receiver projection via the upstream Sutra `axon_project` primitive), and `SutraService` that compiles real `.su` source through the bundled Sutra compiler and runs `on_axon(vector) -> vector` on real torch tensors. 70 tests pass including 4 with real Sutra-compiled services running through the kernel router. The Python is the **behavioural reference** for the production Rust orchestrator (per `planning/01-architecture.md` § "CPU side: small, Rust, orchestrator"); it is **not** the bootloader, which is necessarily Rust because the boot path runs before any interpreter exists (`planning/19-boot-sequence.md`).
- **Sutra-side primitives Yantra depends on, all shipped.** Sutra v0.3.3: compile-time axon-keys static analysis. Sutra v0.3.4: device-coherence fix in axon ops. Sutra v0.3.5: `axon_project(payload, requested_keys)` per-receiver projection primitive. Sutra v0.4.0: `MultiProcessRuntime` — N programs sharing one `_VSA`, one codebook, one embedding cache, one GPU device. Yantra wires all of these into the kernel router and the SutraService.
- **First userspace utility (`apps/echo/`).** Real Sutra source admitted by the kernel through the same path kernel-services use. Smallest possible v0 of the command-line milestone (per `planning/18-kernel-browser-readiness.md` § "Build sequence" step 2). Routes the input axon's `stdin_text` key to the output's `stdout_text` key; pure axon round-trip, no real stdin/stdout streams (those wait on Sutra's IO + FS vocabulary maturing).

Seventeen design documents under `planning/` now cover the corpus end-to-end, including the new boot-sequence (`19-`) and lazy-axon-evaluation (`20-`) docs added during the implementation work. The TS→Sutra transpiler ships as Sutra v0.3.2 (CLI works; 17 fixtures pass). The C→Sutra transpiler is genuinely a skeleton (~57 lines of CLI scaffolding) and stays deferred until kernel + utilities mature.

What's still **not yet started**: the bootloader (Rust), the production Rust orchestrator (Python prototype is the reference), per-process GPU memory arena carve-outs (needs CUDA stream isolation work), eviction-to-RAM cold-store, real disc/RAM/GPU storage-tier moves, and the GUI/browser layer. These are the third milestone in the build sequence; they wait on milestones 1 (Connectome Manager) and 2 (command-line utilities) further maturing.

### 8.2 Milestones to first useful prototype

Yantra rides on two Sutra-side artifacts that already exist and are independently maturing: **the Sutra language and compiler** (the empirical foundation for the axon model and verification surface) and **the TypeScript → Sutra transpiler** (the on-ramp that makes "everything is a browser" workable — without it, Yantra's userspace is empty for the eventual GUI tier). The Yantra-specific work is what sits on top of those.

In rough order of "must work" to "would be nice", with what's shipped marked:

1. **Sutra runtime with fixed allocations** — *partially shipped.* Sutra v0.4.0's `MultiProcessRuntime` runs N programs in one process sharing one `_VSA`. Per-process **GPU memory arena carve-outs** are still upstream (need CUDA stream isolation, possibly CUDA IPC); admission tracks `compute_units` as bookkeeping until those land.
2. **Axon-based IPC** — *shipped.* `kernel/router.py` routes axons between admitted processes with capability check on send + receive, lazy-skip filtering when keys don't intersect, and per-receiver projection via the Sutra `axon_project` primitive. The rotation-operator-based capability check is name-trusted in v0.0; operator-based check lands when the `.su` loader formalises operator carriage.
3. **Init + resource manager** — *partially shipped (Python prototype).* `kernel/init.py` admits processes against a fixed pool, refuses cleanly on exhaustion, never throttles admitted services. The eventual Rust port is the production target; the bootloader is its own separate Rust binary (per `19-boot-sequence.md`). MMIO + interrupt + tick batching (§3.5 of this paper) is unbuilt, blocked on having target hardware.
4. **First userspace utility** — *shipped (echo).* `apps/echo/` is a real Sutra `.su` program admitted by the kernel through the same path kernel services use. The next utilities (cat, ls, wc, grep, …) wait on Sutra's string + IO + FS vocabulary maturing.
5. **GUI / browser** — *not started.* Third milestone after (1) Connectome Manager hardens past v0.0 and (2) command-line utilities mature. HTML5 + CSS + idiomatic TS + WebGL/Three.js. WASM eventually but not for a long time.
6. **Memory model** — *long-pole open problem.* Process scratchpads, eviction granularity, disc↔RAM↔GPU storage-tier moves, MMIO ping-pong cost, cold-store integrity. Tracked in `planning/17-memory-model.md`. Not a v1 milestone.
