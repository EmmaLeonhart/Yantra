...
- **Bootloader (Rust) — v0.1 shipped + verified in QEMU 11.0.50 (2026-05-14).** The first Yantra-authored code that runs at boot. Lives at `bootloader/`. v0.1 boots via multiboot1, enumerates the PCI bus (sees the GPU at 00:02.0 class 0x03 + the other QEMU-emulated devices), transitions to long mode (PML4+PDP identity-mapped 1 GiB, PAE, EFER.LME, far-jump to 64-bit code segment), and prints "Long mode active" from 64-bit asm. **v0.2+ work**: switch Rust target to x86_64 + multiboot2 (so we can run real Rust in 64-bit mode), actual GPU init (program registers, set framebuffer), load the compiled Sutra kernel image into GPU memory, hand control to the orchestrator. See `planning/19-boot-sequence.md` § "Stage 3" and `bootloader/README.md` "What v0.1 does NOT do" for the deferred list.
