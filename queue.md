...
### 🚚 Yantra → Sutra migration — phases 1 & 2 DONE; phase 3 pending design

Phase 1 (apps/font) and phase 2 (apps/gui) landed Sutra-side 2026-05-28
under `external/Sutra/demos/{font,gui}/`. Yantra-side cleanup commits
`327ab4d`, `f40f58f`, `82f9022` removed the duplicates and bumped the
pin. Sutra-side now also carries the bound-vector rewrite finished work
(antipodal-filler encoding at runtime_dim=256, 28× faster than the
original; `font_demo.py` wired to it per Emma's product call) and the
daily-audit GH workflow + demos-ci workflow.

**Phase 3 — remaining migrations (NEEDS DESIGN, not autonomous):**
`apps/calc/`, `apps/echo/`, `apps/terminal/`. These use the Yantra
kernel (`kernel.Manifest`, `kernel.SutraService`, `kernel.PythonService`,
`kernel.router.Axon`). Migrating them cleanly requires either
(a) re-architecting them to call `compile_su` directly + admit-shim, like
font/gui do — that means losing the kernel-router test coverage these
apps provide today, or (b) moving the relevant kernel pieces along to
Sutra. Emma decision.
