...
>
> **Refreshed 2026-05-24:** submodule now pinned at `external/Sutra` **v0.6.2** (adds the `dot` builtin + selectable `runtime_dtype`; the calc now selects on the substrate via `select` and runs float64, exact to 2⁵³). Measured this date on the **real CUDA box (RTX 4070, `torch.cuda.is_available()` True)**: `test_kernel_gpu_tiers.py` **passes 4/4 in isolation**, and a fresh-process probe shows **admit allocates +712,704 B of GPU memory** for `echo` (`_VSA.device == cuda`) — so **admit → GPU-resident works; the DISC↔GPU residency capability is real, not blocked.** In the *whole-suite* run it reads **117 passed, 1 failed, 1 xfailed**, where the **1 failed** (`test_admit_makes_program_gpu_resident`, "+0 B on admit") is a **test-isolation artifact, not a defect**: by the time `gpu_tiers` runs after the other modules, the shared per-process substrate tensors (embedding model + codebook, loaded once per process) are already resident, so admitting `echo` reuses them and the admit-delta reads ~0. Fix is a test-isolation fix (warm/zero the substrate baseline, or assert residency another way), queued — NOT a capability gap. **Earlier drafts of this very line wrongly called it an "unbuilt per-process GPU arena" — that was a bad assumption about the machine; corrected here against measurement.** The **1 xfailed** is the cross-program `axon_project` no-op.
>
