...
## The bubble-economy angle

A practical aside about the business environment:

The current AI hype cycle is driving GPU prices to historic highs.
The contrarian angle: a credible analog computing pivot, executed
counter-cyclically, becomes more attractive precisely when the digital
GPU paradigm hits a wall (cost, power, physical limits).

Yantra is positioned for both worlds:

- During the bubble: sells as a high-end critical-systems OS for
  customers who want predictable AI integration on commodity GPU
  hardware.
- After the bubble: sells as the natural OS for the next-substrate
  hardware that emerges from the post-digital-GPU transition. See
  `13-hardware-roadmap.md`.

This is the long version of "build the human capital and the IP
during the bubble; deploy both during the bust."
