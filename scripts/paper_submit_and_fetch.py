...
Shared between `.github/workflows/submit-papers.yml` (push-triggered
auto-submission) and any local invocation. Pulled out of the inline YAML
script so the workflow shares one implementation, the logic can be
unit-tested locally, and review-fetching isn't trapped inside a heredoc.

Reads paper content from `<paper-dir>/paper.md`, optionally reads
`<paper-dir>/SKILL.md` as supplementary material, extracts the
`## Abstract` section (falling back to the first 500 characters if there
is no such section), and POSTs to clawRxiv's `/api/posts` endpoint.

After submission succeeds, polls `/api/posts/{post_id}/review` every
`--poll-seconds` seconds until a review is returned or
`--review-timeout-seconds` elapses. On success, writes the review JSON
and a rendered Markdown copy into
...
Adapted from the Alignment repo's identically-named script. Kept
behaviourally identical so the same CI pattern works across all our
clawRxiv-pushing projects.
"""
...
def read_paper(paper_dir: Path) -> tuple[str, str | None, str]:
    ...
    # Extract the abstract section — content between `## Abstract` and the
    # next H2 heading, regardless of heading text. Falls back to first 500
    # chars if no abstract section is found. Match any `\n## ` to avoid
    # the digit-prefix trap that bit the Alignment paper (see commit
    # b3e5320 in that repo's history).
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help="Path to paper directory containing paper.md (e.g. paper)",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Comma-separated tag list "
             "(e.g. operating-systems,neuro-symbolic,gpu)",
    )
    parser.add_argument(
